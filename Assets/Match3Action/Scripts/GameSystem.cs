...
public	bool isDoing = false;
	...
	{//print("call");
		...
		for (int i=0; i<sounds.Length; i++) 
		{
			...
			audioMatchSource[i].playOnAwake=false;
	...
	{
		foreach(Transform Temp in grid.transform)
		{
			GameObject TempGO;
...
//    }
	...
	void CheckMatch3(Dictionary<TilePoint, Data.TileTypes> stack)
	{
		...
		foreach (MatchItem item in destroyList)
		{
			...
			{
				PAM.DecreaseHealth(0.1f);
			}
			else
			{
				PAM.IncreaseHeath(0.05f);
				//incrrease health
			}
...
public	void ReadyGameTurn(){
	...
	void Start ()
	{
		...
		effectArea = GameObject.Find ("EffectArea").transform;
		choice = GameObject.Find ("Choice").GetComponent<SpriteRenderer> ();
		PAM = GameObject.Find ("01PlayArea").GetComponent<PlayAreaManager> ();

		InitTheGameSystem ();
	}
	public void InitTheGameSystem()
	{
